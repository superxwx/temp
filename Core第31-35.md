...
正如您所见到的一样，您可以通过  [`CLI.create`](https://vertx.io/docs/apidocs/io/vertx/core/cli/CLI.html#create-java.lang.String-) 方法来创建一个新的 [`CLI`](https://vertx.io/docs/apidocs/io/vertx/core/cli/CLI.html) 。此处传入的字符串参数就是这个CLI的名称。创建之后，可以给它设置摘要和描述。

As you can see, you can create a new 
CLI
 using 
CLI.create
. The passed string is the name of the CLI. Once created you can set the summary and description. The summary is intended to be short (one line), while the description can contain more details. Each option and argument are also added on the CLI object using the 
addArgument
 and 
addOption
 methods.
